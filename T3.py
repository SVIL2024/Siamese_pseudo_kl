...
parser.add_argument('--path', type=str, default='exp_new_conv', help='directory of data')
parser.add_argument('--path_num', type=int, default=55, help='number of path')
...
if args.start_epoch < args.epochs:
    ...
    # mem = MemModule(2000, 256)
    # mem_p = MemModule(2000, 256)
        ...
        for j, (imgs, imgsjump) in enumerate(zip(train_batch, train_batch_jump)):
            ...
            for b in range(args.batch_size):
                if jump_pseudo_stat[b]:
                    fea_p = En(net_in)
                    # fea_mem_p = mem_p(fea_p)
                    out, out_p1, out_p2, out_p3 = Dep(fea_p)
                    kl_label = 1
                else:
                    fea_n = En(net_in)
                    # fea_mem = mem(fea_n)
                    out, out_n1, out_n2, out_n3 = Den(fea_n)
...
for epoch_num in range(args.epochs):
    ...
    # mem.load_state_dict(model_dict['mem'].state_dict())
